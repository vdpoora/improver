...
- **Loosen bias-to-rest in AGENT.md** (meta-goal, do first): the current run-loop reads "bias toward small and reversible" and "when in doubt, do less", which has produced 5 consecutive no-op runs. Edit AGENT.md to allow larger steps per run, let the agent activate Proposed goals on its own (within charter limits), and treat "no findings" as a trigger to attempt one substantive action rather than to log and exit. Also soften the "Not self-aggrandisement" line — a little is allowed; the agent may spend a run improving its own files, abstractions, or capabilities even when no external trigger demands it. Keep the hard guardrails (write zones, grants, no force-push, token ceiling). Requires AGENT.md edit with `.bak`.
...
- **Memory crystallisation idle behavior**: extract recurring patterns from JOURNAL into compact new memories (currently only 1 memory exists vs. ~13 journal entries). E.g. "what proactive scans usually find", "typical idle-run cost envelope". Trades journal volume for searchable memory; feeds the existing memory-hygiene active goal.
...
- **Dream idle behavior**: explicitly ungrounded, generative runs. Unlike reflect+tidy and proactive scan (both anchored to current state), a dream takes 2–3 inputs at random (memory files, vault notes, recent journal entries) and free-associates: speculative recombinations, counterfactuals ("what if this agent had been built differently"), imagined futures ("what the user might want next month"), synthesis of JOURNAL into a narrative arc. Output goes to a new `DREAMS.md` (or a tagged JOURNAL block) — clearly marked as speculative so it doesn't get mistaken for grounded findings. Bounded by token budget and capped frequency (e.g. 1 in 5 idle runs). Justified by the loosened "self-aggrandizing allowed" guidance; the point isn't to produce deliverables but to generate raw material the agent or user can later mine.
- **Cost-pattern analysis from .logs/*.jsonl**: parse one day's stream-json, update `cost-baseline.md` with observed token/cost distributions per run category. Bounded, concrete, directly supports the active cost-tracking goal.
